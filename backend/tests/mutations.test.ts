    ...
    test(
      ...
      runner(async ({ context }) => {
        ...
        expect(errors![0].message).toEqual(
          "Sorry! You must be signed in to create an order!"
        );
      })
    );
    test(
      ...
      runner(async ({ context }) => {
        ...
        expect(result1.data!.checkout.items[0].description).toEqual(
        ...
        expect(result1.data!.checkout.items[0].photo.id).toEqual(
          product1.photo.id
        );
        expect(result1.data!.checkout.items[1].name).toEqual(product2.name);
        expect(result1.data!.checkout.items[1].description).toEqual(
        ...
        expect(result1.data!.checkout.items[1].photo.id).toEqual(
          product2.photo.id
        );
  ...
  describe("addToCart(productId)", () => {
    ...
    test(
      ...
      runner(async ({ context }) => {
        ...
        expect(errors).toHaveLength(1);
        expect(errors![0].message).toEqual(
          "Input error: Only a cuid can be passed to id filters"
        );
      })
    );
